...
double sumFloat(double a, double b) {
    return a + b;
}
...
void print(int count, ...) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        switch (val.type) {
            case LOTUS_BOOL: printf("%s", val.value.i64 == 0 ? "false" : "true");
                break;
            case LOTUS_F32:
            case LOTUS_F64: printf("%f", val.value.f64);
                break;
            case LOTUS_I64:
            case LOTUS_I32: printf("%ld", val.value.i64);
                break;
            case LOTUS_STRING: const LotusString *str = val.value.ptr;
                printf("%s", str->data);
                break;
        }
    }
...
bool toBool(const LotusValue value) {
    switch (value.type) {
        case LOTUS_I32:
        case LOTUS_I64: return value.value.i64 > 0;
        case LOTUS_F32:
        case LOTUS_F64: return value.value.f64 > 0;
        case LOTUS_BOOL: return value.value.i64 != 0;
        case LOTUS_STRING: if (strcmp(value.value.ptr, "true") == 0)
                return true;
...
int toInt(LotusString *str) {
    return atoi(str->data);
}

void hello(LotusString *str, int age) {
    printf("Hello %s! With age %d\n", str->data, age);
}

LOTUS_NATIVE_LIBRARY {
    LOTUS_FUNCTION(hello, LOTUS_VOID, LOTUS_STRING, LOTUS_I32);
