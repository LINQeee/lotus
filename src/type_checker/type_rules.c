...
bool areComparable(LotusType left, LotusType right) {
  ...
  if (left == LOTUS_BOOL && right == LOTUS_BOOL)
    return true;

  if (left == LOTUS_STRING && right == LOTUS_STRING)
    return true;

  return false;
}

bool isValidBinaryOperation(TokenType operator, LotusType left,
                            LotusType right) {
  switch (operator) {
  case TOKEN_PLUS:
    if (isNumericType(left) && isNumericType(right))
  ...
  case TOKEN_EQ:
  case TOKEN_NEQ:
    return areComparable(left, right);

  case TOKEN_AND:
  case TOKEN_OR:
    return left == LOTUS_BOOL && right == LOTUS_BOOL;
...
LotusType binaryResultType(TokenType operator, LotusType left,
                           LotusType right) {
  switch (operator) {
  case TOKEN_PLUS:
    if (left == LOTUS_STRING && right == LOTUS_STRING)
